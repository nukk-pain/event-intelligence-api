...
- **Solar가 프로덕션 두뇌 (기본값)**: 출시 기능의 에이전트 추론(소스 발견·멀티홉·
  판별·교차검증)은 Solar로 돌린다. 로컬 Qwen3.6는 **벤치마크 대조군**일 뿐 출시
  경로의 폴백이 아니다. Upstage 프로그램은 Solar 채택·쇼케이스가 목적이므로, 자세는
  "Solar 위에서 만든다"이지 "Solar를 우회한다"가 아니다.
- **비용 캐스케이드 = 낭비 방지**: 결정적 파서(무료)로 뻔한 정형 페이지를 먼저
  거르는 건 LLM 낭비를 막는 상식일 뿐, Solar 호출을 줄이려는 목적이 아니다.
  어렵고 비정형인 핵심 작업은 전부 Solar가 한다.
- **A/B의 목적 = Solar 한국어 강점 입증**: 로컬 대조군과의 비교는 "Solar를 쓸지
  판단"이 아니라 한국어 추출에서 Solar의 우위를 계량화해 후기·근거로 쓰기 위함.
  결과는 정직하게 기록(미리 승패 단정 금지).
