...
- **(해결됨 2026-07-27)** seed 계정 불일치(6개 중 5개만 집계)의 원인은 robots
  부트스트랩의 조기 반환이었다. strict 모드는 robots.txt를 읽지 못한 origin을 크롤하지
  않는데, 그 seed는 HTML 큐에 들어가지도 못한 채 아무 집계 없이 버려졌다. 이제
  `robots_unavailable`로 계정되어 합계가 카탈로그 크기와 일치한다.
- 라이브에서 매 실행 1건 정도가 `robots_unavailable`로 빠진다. 수동 확인 결과 6개
  seed의 robots.txt는 전부 200을 반환하므로 영구 차단이 아니라 60초 크롤 예산 안의
  일시적 실패로 보인다. robots를 못 읽으면 크롤하지 않는 것은 의도된 안전 동작이므로
  교정하지 않는다. 계측만 남긴다.
