...
# Only a seed-protocol candidate is guaranteed a title, so the seed count is
# what separates a crawl that never reached a seed page from model rejection.
if ! grep -Fxq 'crawl_seed_candidates=1' "$PRESENT_REPORT" ||
  ! grep -Fxq 'crawl_validated_candidates=2' "$PRESENT_REPORT" ||
  ! grep -Fxq 'crawl_skipped_documents=3' "$PRESENT_REPORT" ||
  ! grep -Fxq 'crawl_truncated=true' "$PRESENT_REPORT" ||
  ! grep -Fxq 'crawl_truncation_reasons=protocol_document_limit,time_limit' "$PRESENT_REPORT"; then
  fail 'crawl_summary_present=false'
fi
...
if ! grep -Fxq 'crawl_seed_outcome_robots_disallowed=2' "$PRESENT_REPORT" ||
  ! grep -Fxq 'crawl_seed_outcome_body_too_large=2' "$PRESENT_REPORT" ||
  ! grep -Fxq 'crawl_seed_outcome_candidate=1' "$PRESENT_REPORT"; then
