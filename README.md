...
이 저장소에서 Solar가 가장 주체적으로 일하는 곳은 소스 발견 루프인
`eventscout`입니다. 처음에는 검색과 판정의 순서를 코드가 고정하고 Solar는
빈칸만 채웠습니다. 제출 직전에 그 구조를 뒤집었습니다. 지금은 매 턴 Solar가
검색, 후보 페이지 열람, 채택, 종료 중 하나를 고르고 루프는 그것을 실행할
뿐입니다. 코드에 남긴 것은 예산과 경계입니다. 어디를 파고 언제 멈출지는
모델이 정합니다.

뒤집은 날 운영 서버의 첫 실행에서 Solar는 여덟 턴을 전부 스스로 끌고
갔습니다. 검색으로 후보를 모으고, 확신이 없는 페이지 3곳을 열어 근거를 확인한
뒤 공개 소스 10건을 채택했습니다. 순서를 코드가 정하던 시절의 같은 조건
수율은 5건이었습니다. 출력 토큰은 여덟 턴을 합쳐 590개입니다.

여기까지 오는 길은 곧지 않았습니다. 이 루프는 한동안 빈손이었고, 짐작으로
고치는 대신 후보가 어느 단계에서 사라지는지 개수로만 남기는 계기판부터
만들었습니다. 가설이 두 번 틀렸고 두 번 다 계측이 진짜 원인을 알려줬습니다.
판정 품질도 따로 쟀습니다. 실제 웹 검색이 돌려준 12건을 주자 공식 행사 소스
8건은 전부 받아들이고 산업 보고서와 입시 안내 같은 오답 4건은 전부 걸렀습니다.

발견이 데모에 머물지 않게 승격 경로를 붙였습니다. Solar가 채택한 후보를
사람이 검토해 커밋으로 크롤 대상에 넣는 구조입니다. 이 경로로
인공지능산업협회 게시판과 전국 전시회 디렉토리가 실제 수집 소스가 됐고, 두
소스가 더한 행사만 215건입니다. 전체 데이터셋은 909건이 됐고 지방 전시장
행사가 처음 커버리지에 들어왔습니다. 주간 발견은 서버 타이머로 계속 돕니다.
크롤 경계를 바꾸는 마지막 한 수만 사람 몫으로 남겼는데, SSRF 방어선을 리뷰된
코드 밖으로 내보내지 않기 위해서입니다.

같은 저장소의 나머지 두 갈래도 Solar로 돕니다. 멀티홉 조사 루프는 배포 중인
수집 경로에서 결정적 파서가 못 채운 참가 정보를 216건 중 40건에서 채웠고, MCP
서버는 자연어 질문을 필터로 바꿔 라이브 행사 API를 조회합니다. 모델이 채운 값
61개는 공식 페이지와 하나씩 대조했고, 근거율이 낮았던 마감일은 읽은 페이지에
날짜가 실재할 때만 저장하도록 바꾼 뒤 이전 값을 회수했습니다. 모델이 채웠다는
사실과 맞게 채웠다는 사실은 다른 문제였습니다.

모델 자체에 대한 발견도 있습니다. `solar-open2`는 `reasoning_effort`를
생략하면 짧은 JSON 요청에도 숨은 추론을 길게 씁니다. completion 예산이 작을
때는 추론에만 다 쓰고 본문이 빈 채 끝나기도 했습니다. `minimal`을 명시하자
같은 요청의 추론 토큰이 726에서 0으로 떨어졌고, 그 위에서 호출당 예약을
500토큰까지 줄여 같은 비용 상한 안에서 턴 수를 두 배로 늘렸습니다. 답의
모양이 정해진 일에서는 더 오래 생각하게 하는 것보다 추론 예산을 정확히
통제하는 편이 품질과 비용을 함께 잡습니다.
