...
1. `eventscout`는 목표를 받아 검색어를 제안하고 후보 소스를 판정합니다.
...
Solar는 데모용으로 두지 않고 운영 중인 서비스에 넣었습니다. 소스를 찾는 탐색
루프, 행사 하나를 파고드는 조사 루프, 자연어로 물어보는 MCP 서버 세 갈래입니다.
두 번째 루프는 배포되는 수집 경로에서 돌고, 결정적 파서가 못 채운 참가 정보를
216건 중 40건에서 채웠습니다. 대조 실행과 비교해 잰 값이라 크롤 편차와 섞이지
않은 숫자입니다.

소스 발견 루프는 한동안 빈손이었습니다. 유실 지점을 개수로 분해해 원인을 찾고
고친 뒤에는 공개 소스 5건을 발견하고 판정합니다. 가설이 두 번 틀렸고, 두 번 다
계측을 보고 고쳤습니다.

채운 값의 정확도는 61개를 하나씩 공식 페이지와 대조해 확인했습니다. 영문명은 잘
맞고 마감일은 근거율이 낮아서, 읽은 페이지에 날짜가 실재할 때만 저장하는 검증을
넣고 이전 값은 회수했습니다. 모델이 채웠다는 사실과 맞게 채웠다는 사실은 다른
문제였습니다.

고정 fixture 실험에서는 로컬 대조군과 Solar 둘 다 54개 필드를 전부 맞혔고
출력 토큰은 2,494에서 118로 줄었습니다. 다만 이 차이는 대부분 추론 예산 설정에서
나온 것이라 Solar가 스무 배 효율적이라고 읽으면 과합니다. 표본 6건은 동작 확인
수준이고, 우열을 가리는 재측정은 아직 하지 않았습니다.

베타가 끝나기 전에 발견 루프를 운영에 붙였습니다. Solar가 판정한 후보를 사람이
검토해 코드로 승격하는 경로를 만들었고, 그 경로로 인공지능산업협회 게시판과
전국 전시회 디렉토리가 수집 소스가 됐습니다. 두 소스가 더한 행사가 215건, 전체
데이터셋은 909건. 지방 전시장 행사가 처음 커버리지에 들어왔습니다. 주간 발견은
서버 타이머로 계속 돌고, 크롤 경계에 소스를 넣는 마지막 한 수는 사람의 커밋으로
남겨뒀습니다.
