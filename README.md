...
Solar를 제일 세게 쓴 곳은 `eventscout`다. 원래는 검색하고 판정하는 순서를
코드가 정해놓고 Solar한테 빈칸만 맡겼는데, 제출 직전에 뒤집었다. 지금은 매 턴
Solar가 검색할지, 후보 페이지를 열어볼지, 채택할지, 그만할지 고른다. 코드는
예산이랑 SSRF 경계만 지킨다.

뒤집은 날 운영 서버에서 바로 돌려봤다. 여덟 턴을 혼자 끌고 가면서 애매한
페이지 세 곳을 열어 확인하고 공개 소스 10건을 채택했다. 순서를 코드가 정하던
때는 5건이었다. 출력 토큰은 다 합쳐 590개.

처음부터 이랬던 건 아니고 한동안은 빈손이었다. 어디서 새는지 몰라서 후보가
사라지는 지점을 개수로 남기는 계기판부터 만들었는데, 가설이 두 번 틀렸고 두 번
다 계측이 맞았다. 판정 자체는 믿을 만하다. 실제 검색 결과 12건을 주니 공식
소스 8건은 다 받고 보고서·입시 안내 같은 오답 4건은 다 걸렀다.

채택된 후보는 사람이 검토해서 커밋으로 크롤 대상에 넣는다. 이 경로로
인공지능산업협회 게시판과 전국 전시회 디렉토리가 들어왔고, 둘이 더한 행사만
215건. 덕분에 지방 전시장 행사가 처음 잡히기 시작했다. 마지막 커밋을 사람
손에 남긴 건 SSRF 방어선을 리뷰 안 거친 코드로 내보내기 싫어서다.

수집 쪽에도 Solar가 돈다. 결정적 파서가 못 채운 참가 정보를 216건 중
40건에서 채웠고, 채운 값 61개를 공식 페이지랑 일일이 대조해서 근거가 약한
마감일은 저장 조건을 조이고 이전 값을 회수했다. MCP 서버는 자연어 질문을
필터로 바꿔 라이브 API를 조회한다.

`solar-open2` 쓰면서 하나 배운 것. `reasoning_effort`를 안 주면 짧은 JSON
하나 받는 요청에도 숨은 추론을 길게 쓴다. 예산이 작으면 추론만 하다가 본문
없이 끝나기도 한다. `minimal`을 박으니 추론 토큰이 726에서 0이 됐고, 그 덕에
호출당 예약을 500토큰까지 줄여서 같은 비용으로 턴을 두 배 돌린다.
