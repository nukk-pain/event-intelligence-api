...
Solar를 데모 하나가 아니라 운영 중인 서비스 옆에 붙여 봤습니다. 소스를 스스로
찾는 루프, 행사 하나를 링크 따라 파고드는 루프, 다른 에이전트가 자연어로 물어보는
MCP 서버까지 세 갈래입니다. 이 중 소스 발견 루프는 한동안 빈손이었습니다. 유실
지점을 개수로 분해해 원인을 찾고 고친 뒤에는 공개 소스 5건을 실제로 발견하고
판정합니다.

고정 fixture 벤치마크에서는 로컬 대조군과 Solar 둘 다 54개 필드를 전부 맞혔고
출력 토큰은 2,494에서 118로 줄었습니다. 다만 이 격차의 상당 부분은 추론 예산
설정 차이라 Solar가 스무 배 효율적이라고 읽으면 과합니다. 이 표본으로 확인한 건
한국어 구조화 추출에서 Solar가 짧고 빠르게 답한다는 사실이고, 우열까지 가리려면
조건을 맞춘 재측정이 필요합니다.
