...
func applyActionSignals(e *model.Event, p *sources.ParsedEvent, mf *missingFields) {
	...
	applyDateSignal(p.Actions.RegistrationDeadline, &e.RegistrationDeadline, "registration_deadline", mf)
	applyDateSignal(p.Actions.ExhibitorDeadline, &e.ExhibitorDeadline, "exhibitor_deadline", mf)
...
func applyDateSignal(signal *string, dst **string, field string, mf *missingFields) {
	if signal == nil {
		mf.add(field)
		return
	}
